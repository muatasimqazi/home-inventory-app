 ...
 * Reprocess existing cutouts too:
 * pnpm dlx tsx scripts/backfill-item-background-removal.ts --replace-existing
 */
...
async function main() {
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    ...
    if (!REPLACE_EXISTING) query = query.is("background_removed_photo_path", null);
  ...
  console.log(`Found ${items.length} items to ${REPLACE_EXISTING ? "reprocess" : "process"}.\n`);
  ...
  for (const item of items) {
    ...
    try {
      ...
      let updateQuery = supabase.from("items").update({ background_removed_photo_path: path }).eq("id", item.id);
      if (!REPLACE_EXISTING) updateQuery = updateQuery.is("background_removed_photo_path", null);
