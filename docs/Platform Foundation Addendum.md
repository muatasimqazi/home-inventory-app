...
**Updated 2026-08-18 — this section's own history is the cautionary example, not just the rule.** Finance's privacy model flip-flopped three times over this project: v1 shipped a per-record visibility layer (invented, not requested); v2 corrected it back to Shohaz's household-wide-shared default (reasoned as "matches discovery of the sibling PRD"); v3 — the actual, final, user-confirmed answer — reversed it a third time back to real per-record privacy with opt-in sharing, because that is literally what the user asked for when directly asked ("individuals within a house can have their own finance details... or can share with household," confirmed via AskUserQuestion as "Real privacy, opt-in sharing"). Full design: Personal Finance Addendum, "Privacy model"; schema: PRD §10, §32.8.

The standing rule survives this history, just pointed the other way: **a new domain inherits the existing conventions (household-shared RLS, Archive+Trash lifecycle, activity-log audit) by default — except financial-data visibility, which is a confirmed, deliberate per-domain divergence for Finance specifically** (Owner/Member stay equal for household *administration* — invite, remove, roles — per §6 of the Finance PRD; they are *not* equal for financial-data visibility, which is per-account/per-bill and owner/share-based). A future domain should not assume Finance's privacy layer is the default to copy, nor assume Shohaz's flat sharing is the default either — each new domain should state its own answer explicitly and get it confirmed, the same way this document is naming platform conventions explicitly rather than letting them drift implicitly. The lesson from three reversals: state the answer plainly in the doc every time it changes, and don't treat an earlier "corrected back to match" as more authoritative than a direct answer to a direct question.
