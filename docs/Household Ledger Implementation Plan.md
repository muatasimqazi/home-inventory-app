...
**Status: done.** `0017_household_ledger_core.sql` through `0021_drop_item_owner_user_id.sql` were all verified locally first, then pushed to the linked production project via `supabase db push --linked` (dry-run confirmed before each push — exactly one pending migration each time, nothing unexpected). `0021` (a real column drop) was confirmed with the user before pushing, unlike the purely additive migrations before it. `supabase migration list` shows local and remote both at `0021`.
