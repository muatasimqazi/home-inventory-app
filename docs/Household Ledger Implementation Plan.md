...
- A dead prop: `CategoryExtraField.placeholder` (the "YYYY-MM-DD" hint defined in `category.ts` for `warrantyEnd`/`expirationDate`) is never actually read — `add/page.tsx` hardcodes every extra-detail input's placeholder to `field.label` instead. Cosmetic, low priority.
...
**Bug-fix pass — status: done for the four real correctness bugs; two low-priority items deliberately left.** All four were independently re-verified against the current code before fixing (not taken on faith from the Wave 2 review that surfaced them), and three of the four were confirmed against the real running app, not just `tsc`/lint:
  ...
  - **Left deliberately, both low-priority:** the warranty-active NaN-guard duplication between `item-purchase-section.tsx` and `ask/tools.ts` (maintainability debt, not a bug — both copies are correct), and a dead `CategoryExtraField.placeholder` prop found during the integration pass (§9a) — defined in `category.ts` but never read; `add/page.tsx` hardcodes every extra-detail input's placeholder to `field.label` instead.
