...
| # | Workstream | PRD §§ | Primary files | Depends on |
|---|---|---|---|---|
| 1 | **Onboarding rewrite** | 11–22 | `src/app/household-setup/page.tsx`, `src/app/sign-in/` | Phase 0 (`people` table) |
| 2 | **People & ownership UI** | 8, 9, 23, 30 | `src/app/(shell)/settings/members/` → generalize to People; `AddPersonSheet` shared component (new); `src/components/item-ownership-section.tsx` (Phase 0) | Phase 0 |
| 3 | **Finance ↔ item linking** *(core — see note)* | 25 | `src/components/item-purchase-section.tsx` (Phase 0, currently a no-op); new "Link to item" affordance in `src/app/(shell)/finance/transactions/`, `src/app/finance/scan/review/`; extend `src/lib/ask/tools.ts` | Phase 0 (`item_purchases` table) |
| 4 | **Documents surfacing** | 28 | `src/components/item-attachments.tsx` (already exists, already isolated — confirm upload/view flows are complete against `attachments.kind`; do not add new kinds) | None — schema-complete, UI-complete component already exists; likely the fastest workstream to close out |
| 5 | **Simple Home Map** | 29 | New route under `src/app/(shell)/`, e.g. `home-map/`; capture flow reusing existing camera components | Phase 0 (`pinned_locations` table) |
...
`AddPersonSheet` (workstream 2) is a dependency for workstream 1's contextual "add someone" moment (§22 of the PRD). Fastest path: workstream 2 defines and lands the component first (it's small), or both agents agree on a two-line prop interface up front and build against a stub — either avoids blocking.
...
- **`owner_user_id` deprecation timeline** — 56 call sites across `src/` read or write `items.owner_user_id` today (confirmed by grep before writing the migration). It's kept as a compatibility shim, not dropped — Workstream 2 migrates call sites to `owner_person_id`, then a follow-up migration drops the old column once none remain. Re-grep before dropping, not after Workstream 2 claims it's done. **Drift risk closed:** `0018_owner_sync.sql` extends `sync_item_location()` so writing either column auto-derives the other (same pattern as the existing `location_id`/`container_id` sync), with a guard that rejects a write naming both columns inconsistently. Verified against real local data: owner_user_id-only, owner_person_id-only, neither, and a deliberate mismatch all behave correctly. This means the 56 unmigrated call sites can't silently produce items with `owner_person_id = null` while Workstream 2 is still in progress — both columns stay correct regardless of which one a given code path writes.
- **Managed-profile-to-account conversion** — the audit flagged this as a real consent/identity question (what happens to a managed child's history when they turn 18 and want their own account), not just an engineering one. Phase 0's `people.linked_user_id` (nullable, set only once a profile is linked to a real account) doesn't foreclose an answer either way, but Workstream 2 should treat this as a real open question to design for, not an edge case to skip.
