...
**Status: done.** `0017_household_ledger_core.sql` through `0020_activity_log_person.sql` were all verified locally first, then pushed to the linked production project via `supabase db push --linked` (dry-run confirmed before each push — exactly one pending migration each time, nothing unexpected). `supabase migration list` shows local and remote both at `0020`.
