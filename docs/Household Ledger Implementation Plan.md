...
**Status: done.** #6 and #7 dispatched together (`isolation: "worktree"`, `run_in_background: true`), #8 held back until #7 merged per its dependency and §4's hot-file note. Both #6 and #7 hit account-level interruptions mid-task (a spend-limit reset partway through, then the machine sleeping twice during #7) — resumed each from its own transcript via a direct message rather than restarted, so no work was lost or redone. Merged in the planned order — #6 → #7 → #8 — with `pnpm exec tsc --noEmit` + full-repo lint clean after every merge. Final state: full-repo `tsc --noEmit` and `eslint` both clean.

| # | Workstream | PRD §§ | Depends on | Actual outcome |
|---|---|---|---|---|
| 6 | **Finance-triggered capture nudge** | 26 | #3 (needs `item_purchases` + linking UI to deep-link into) | Shipped as briefed: new `send-capture-nudges` cron job (same shape as `send-due-bills`), qualifying on an amount threshold or a durable-goods merchant/category keyword match (deliberately excluding ambiguous big-box merchants like Costco, per PRD §25's own example), deep-linking into a capture session that auto-links every item saved from it via the existing `linkItemPurchase` action (`source: "finance_nudge"`). `/code-review` before merging surfaced one real bug in the new route (a swallowed `event_notification_log` insert error) — fixed. It also surfaced several **real, pre-existing bugs in already-merged Wave 1 code**, found incidentally while reviewing this branch's use of shared infrastructure — see the new bullet in §9. Not fixed here (deliberately mirrors an existing, already-shipped pattern rather than introducing a new one): the per-transaction sequential loop and the copy-pasted job skeleton both match `send-due-bills` exactly, which the brief explicitly said to follow — reworking that shared pattern would mean also touching a file outside this workstream's scope. |
| 7 | **Appliance scan + lifecycle** | 27 | #2 (ownership), #3 (warranty display via `item_purchases`) | Shipped as briefed: a new, self-contained `/capture/appliance` entry point (own store, own review page) rather than a mode grafted onto the general capture flow — deliberate, so Workstream 8 could route to it cleanly. New `Appliance` category with its own extra-fields set (manufacturer/model/serial/manufacture date/warranty), reusing the exact `warrantyEnd` key `item-purchase-section.tsx` already reads, so warranty display needed zero further wiring. `/code-review` before merging found and fixed three real bugs: a detection failure silently dropped the user back to the crop screen with no error shown (the error-card UI that already existed for this case was unreachable due to a mode-transition bug); the same failure path on retry could render a blank screen; and the camera stream was being stopped before knowing whether detection succeeded, forcing an unnecessary permission re-prompt on retry. All three fixed on-branch before merge. |
| 8 | **Universal Scan narrow launch** (2 modes: item, receipt) | 17, 32 | #7 for the appliance entry point to route around; otherwise light | Shipped, and smaller than briefed once the actual state of things was accounted for: the plan's file-scope note pointed at `src/app/scan/`, but that's actually the QR/barcode *container*-lookup scanner, unrelated to Universal Scan's launch surface — corrected at dispatch time so the agent didn't touch the wrong file. The real chooser (`scan-chooser-sheet.tsx`) already had "Scan Item"/"Scan Receipt" as two explicit, user-picked modes with AI-guess-plus-correction already correctly implemented in both destinations — audited, confirmed correct, nothing needed fixing. Added a third "Scan Appliance" row routing to Workstream 7's entry point, forwarding the same tapped-from location/container context the other two rows already carry. Single-file, 22-line diff. Both an independent manual review and `/code-review` found zero issues. |
...
## 9a. Integration pass — the actual core loop, verified end to end

**Status: passed**, against the real running app, real local Supabase, real sign-in, real Gemini call — not a diff review. Created a real Appliance item (Samsung Refrigerator) via Add Manually in the household that actually has transaction history, set a warranty end date, linked it to a real Whole Foods transaction through the real `LinkPurchaseSheet` UI, then asked the real Ask assistant "Is my Samsung Refrigerator still under warranty?" and got back "Yes, your Samsung Refrigerator is still under warranty until January 1, 2030" — correct, and reasoning over real linked data (`getItemPurchaseInfo` → `item_purchases` → the item's `extraDetails.warrantyEnd`), not a guess. Zero console errors across the whole flow. Test data cleaned up afterward (item, location; `item_purchases` cascaded automatically on item delete, confirming that FK behavior too). **This is the product's actual differentiated bet, and it works.**

Two real bugs surfaced purely by doing this instead of just reviewing diffs — both added to §9 below:

- A dead prop, **fixed**: `CategoryExtraField.placeholder` (the "YYYY-MM-DD" hint defined in `category.ts` for `warrantyEnd`/`expirationDate`) was never actually read — `add/page.tsx` and `items/[id]/edit/page.tsx` both hardcoded every extra-detail input's placeholder to `field.label` instead. These inputs have no separate visible label, so switching to just `field.placeholder` would have traded the field's name for a format hint with no name attached — combined instead: `"Warranty end (YYYY-MM-DD)"`. Verified against the real app.
- **A real, pre-existing, product-wide date bug**, higher priority than anything already in §9: entering warranty end `2030-01-01` displayed as **"Warranty active until Dec 31, 2029"** on the item page, while the Ask assistant correctly said **"January 1, 2030"** for the exact same fact. Root cause: `formatDate()` (`src/lib/format.ts`) does `new Date(iso).toLocaleDateString(...)` — correct for a real timestamp, but a bare date-only string (`"2030-01-01"`, no time/offset) is parsed as **UTC midnight** by the `Date` constructor, then rendered in the browser's **local** timezone, silently shifting back a day for anyone west of UTC. The Ask tool avoids this by never formatting the date at all — it hands the raw ISO string straight to the model, which phrases it correctly. Confirmed the blast radius before fixing anything: `transactions.occurred_at` is a `date` column (`0010_finance_schema.sql`), not a timestamp, so this isn't limited to the new warranty display — `transaction-detail-sheet.tsx` and `link-purchase-sheet.tsx` (both pre-existing, unrelated to this PRD) format that same column through `formatDate()` and are equally affected. `desktop/labels/page.tsx`'s use of `formatDate()` on `batch.createdAt` is fine — that column really is a `timestamptz`.

---

## 9. Open items

- **Onboarding location-creation dead end — fixed.** A brand-new household (zero locations) hit an empty, pick-only `MoveSheet` with no way to add one — the exact gap PRD §14's "type a name, it gets created" was supposed to prevent, never actually wired into `src/app/capture/review/page.tsx`. Fixed once in `src/components/move-sheet.tsx` itself (every consumer benefits, not just capture/review): "+ New location" always visible, plus a "+ New container in `<name>`" per location, both reusing the existing `EntityFormSheet` and auto-selecting the new destination. Verified against the real running app with a genuinely empty household (real sign-in, real local DB, zero pre-existing locations), not just `tsc`/lint — screenshotted at each step, zero console errors.
- **Onboarding "+ Add someone" gap — fixed.** `capture/review/page.tsx` had no ownership picker at all — every scanned item defaulted to Household with no way to assign it to a person, unlike `add/page.tsx`. Added one shared "Belongs to" selector for the whole capture session (same design as the existing shared `destination`), mirroring `add/page.tsx`'s field exactly, wired to the existing `AddPersonSheet`. Verified against the real running app: real sign-in, a real fake-camera capture flow (a synthetic test photo genuinely recognized by Gemini vision as "Tool Box"), used "+ Add someone" mid-scan to create a real managed profile, confirmed it became the selected owner. **Zero-AI-detections crash — fixed** (separate commit from the change that found it). Added an explicit empty state ("We didn't spot anything," Try again / Add manually) rather than letting `SingleReviewForm` render with `row={detections[0]} === undefined`. Verified against the real app with a deliberately blank test photo: zero detections, empty state renders, zero page errors, "Add manually" navigates correctly.

- **`owner_user_id` deprecation — done; column dropped.** The "53 remaining references" were re-grepped and classified one by one: the overwhelming majority were never part of this shim — `accounts.owner_user_id`/`recurring_bills.owner_user_id`, an unrelated, permanent column with the same name (finance's joint-vs-personal privacy model), left untouched. Of the genuinely items-related references, every one was either the shim's own plumbing (mapper, types, trigger) or a write site already setting `ownerPersonId` as the source of truth — zero stale reads. Verified directly against data (zero mismatches) before writing `0021_drop_item_owner_user_id.sql`: `Item`/`NewItemInput`/the mapper/the three write sites all had `ownerUserId` removed, `sync_item_location()` simplified, column dropped (index cascaded with it). Verified against the real app after applying locally — create → edit → assign a managed profile as owner → save, correct at the DB level, zero console errors — then pushed to production (confirmed with the user first, since a column drop is more consequential than the additive migrations before it).
- **Managed-profile-to-account conversion — done.** Asked the user which of three approaches (invite-carries-target-person, a separate claim-code mechanism, or deferring entirely) — chose invite-carries-target-person: `invites.target_person_id` (`0022_invite_target_person.sql`), set only when "Invite to the app" is used *from an existing managed profile's row* (new action in the People page, not the generic "Add someone" flow). Acceptance updates that exact Person row's `linked_user_id` in place rather than inserting a new one, so every item and activity entry already accumulated stays attached — no de-duplication step, ever.

  **A real, more significant bug surfaced while implementing this**, worth calling out on its own: `accept_invite_by_email()` — the function the app actually calls — only ever inserted into `members`, never `people`, since it was born. Workstream 2's people-row-creation logic (0017) was added to `accept_invite(p_invite_id, ...)` instead, a different function the app has never called. Every member who has ever joined a household via a real invite (not `create_household()`'s own path) has been invisible on the People page and unable to own an item since the People feature shipped. Fixed on the function that actually matters, in the same migration.

  Verified against real Postgres end to end, not just `tsc`/lint: a plain invite now creates a `people` row (confirmed it didn't before the fix); a real invite sent through the actual new UI, accepted by a real signed-up user via the real RPC, converted the exact same managed profile in place — one row, not two, `linked_user_id` correct, existing `relationship` preserved. Pushed to production after explicit confirmation, given it rewrites a live security-definer auth function real sign-ups depend on.

**Bugs found during Wave 2 review — pre-existing Wave 1 code, not Wave 2's scope, not fixed.** Wave 2's `/code-review` pass on Workstream 6 read `store.ts`, `ask/tools.ts`, and `household-setup/page.tsx` as context (Workstream 6's new route touches `item_purchases`/`people`/notification preferences) and surfaced real bugs in that already-merged code, outside the diff actually being reviewed. Fixing them would have meant expanding Wave 2's scope into Wave 1 workstreams' territory well beyond the brief — flagged here instead, for a dedicated pass:

  - **Onboarding join flow — fixed.** `household-setup/page.tsx`'s zero-household auto-create effect fired unconditionally whenever `mode === "create"` (the default) and the user had no household yet — it only skipped for `mode === "join"`, which nothing in the app ever set automatically, and the only "I have an invite code" button lived in the `naming` phase, unreachable before a household already existed. A user who signed up specifically to redeem a household invite got a brand-new, unwanted household auto-created and owned by them instead. Root cause the page genuinely couldn't work around alone: RLS on `invites` is "household member read" only, so a brand-new invitee can't SELECT their own pending invite client-side, and the existing `accept_invite_by_email()` RPC is accept-only (mutates on match), not a safe check. Fixed with a new read-only counterpart, `find_pending_invite_by_email()` (`supabase/migrations/0019_pending_invite_check.sql`, same security-definer `auth.email()` lookup, no mutation), a new `checkPendingInvite()` store action (fails open — an RPC error just falls through to the existing no-invite behavior), and a new "invited" phase in `household-setup/page.tsx`: "You're invited to join `<household>`" with explicit Join / "No thanks, set up my own" actions, checked once before auto-create fires. Zero added friction for the common (no-invite) case. Verified against real Supabase Auth, not just `tsc`/lint: two real signups via the Auth REST API (one against a real pending invite, one fresh with none), each round-tripped through the actual RPC with its own real Auth-issued JWT and got back exactly what the store action expects; expired invites, already-accepted invites, and case-insensitive email matching all separately verified via direct SQL transaction tests. Test data cleaned up afterward. **Not yet pushed to the linked production project** — a dry-run confirms exactly one pending migration, left as an explicit decision since this is new, unplanned scope added after Wave 2 rather than part of the migrations §10 already documents as pushed.
**Bug-fix pass — status: done for the four real correctness bugs; two low-priority items deliberately left.** All four were independently re-verified against the current code before fixing (not taken on faith from the Wave 2 review that surfaced them), and three of the four were confirmed against the real running app, not just `tsc`/lint:

  - **`removeMember()` fixed.** Now converts the removed member's Person row to a real managed profile (`linkedUserId` → `null`) as part of the same action, same optimistic + `persistOrRevert` shape used throughout `store.ts`. This also resolves the reported "wrong delete button" symptom as a side effect — once `linkedUserId` is genuinely null, the People page's `deletePerson`-based "Remove" action is the *correct* one for what's now, correctly, a managed profile. Verified by code and schema inspection (a lower bar than the other three below — noted rather than glossed over).
  - **`updatePinnedLocation()` fixed.** `uploadPinnedLocationPhoto()` now writes to a fresh unique path per upload (matching `uploadCoverPhotoFile`'s existing scheme) instead of a fixed per-pin path, so the update can follow `setLocationCoverPhoto`'s already-safe order: upload new → write the DB row → only *then* delete the old object (or clean up the new one as an orphan if the write itself fails). Verified against the real app: replaced a pin's photo, confirmed via direct Storage inspection that exactly one object remains at the new path and the old one is gone.
  - **`addPerson()` fixed.** Converted to the "Real, awaited" shape already used elsewhere in `store.ts` for exactly this class of problem (`setItemCoverPhoto`, `createPinnedLocation`) — insert first, only return/add-to-state once it actually confirms — since its one call site (`AddPersonSheet`) treats the returned id as immediately real (e.g. selecting them in an ownership picker). Verified against the real app — and that's what surfaced a second, previously-unknown bug: adding a person 400'd against `activity_log` every time (`activity_log_entity_type_check` never got `'person'` added when Workstream 2 shipped, even though `0010_finance_schema.sql` had already expanded this exact constraint once before, for the finance domain). Fixed with `0020_activity_log_person.sql`, pushed to production.
  - **Ask-tool `_` escaping fixed.** Consolidated the five independent copies of the same stripping regex into one `escapeSearchInput()` helper (extended to include `_`) rather than fixing the same pattern five separate times — the duplication was itself how the underscore case got missed at four of the five sites. Verified against the real app: asked the real Ask assistant a real spend question after the refactor and got a correct answer, zero console errors.
  - **Warranty-active duplication — fixed.** `item-purchase-section.tsx`'s `isWarrantyActive()` and `ask/tools.ts`'s inline `warrantyStatus` IIFE were two independent copies of the same date-parse-and-compare logic. Consolidated into one `warrantyStatus()` in `lib/selectors.ts` (already the home for small shared derivations, usable from both a `"use client"` component and server-only `ask/tools.ts`). Verified against the real app: an expired-warranty item shows "Warranty expired" on the item page and the real Ask assistant separately says "no longer under warranty... expired on January 1, 2020" for the same item — both from the one shared function.

**Every item raised in this document is now closed.** §9's five items (onboarding location/people gaps, the zero-detections crash, `owner_user_id`, managed-profile conversion, the two low-priority cleanups) and §9a's two integration-pass findings (the date bug, the placeholder prop) were worked through one at a time, each independently re-verified against the real running app or real Postgres before being marked done — not taken on faith from whichever pass first surfaced it. Three real, previously-unknown bugs came out of that verification discipline that no diff review had caught: the timezone date bug, the `accept_invite_by_email()` missing-`people`-row bug, and the `activity_log` constraint gap. Migrations `0017` through `0022` are all applied to both local and production.

---

## 10. Remote deployment

**Status: done.** `0017_household_ledger_core.sql` through `0022_invite_target_person.sql` were all verified locally first, then pushed to the linked production project via `supabase db push --linked` (dry-run confirmed before each push — exactly one pending migration each time, nothing unexpected). `0021` (a real column drop) and `0022` (rewrites a live security-definer auth function) were both confirmed with the user before pushing, unlike the purely additive migrations before them. `supabase migration list` shows local and remote both at `0022`.
