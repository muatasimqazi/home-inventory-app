...
- **Onboarding "+ Add someone" gap — fixed.** `capture/review/page.tsx` had no ownership picker at all — every scanned item defaulted to Household with no way to assign it to a person, unlike `add/page.tsx`. Added one shared "Belongs to" selector for the whole capture session (same design as the existing shared `destination`), mirroring `add/page.tsx`'s field exactly, wired to the existing `AddPersonSheet`. Verified against the real running app: real sign-in, a real fake-camera capture flow (a synthetic test photo genuinely recognized by Gemini vision as "Tool Box"), used "+ Add someone" mid-scan to create a real managed profile, confirmed it became the selected owner. **New edge case found along the way, not fixed (out of scope for this change):** a photo that produces zero AI detections crashes the review page — `isBulk = detections.length > 1` is false when `length === 0`, so it renders `SingleReviewForm` with `row={detections[0]}` = `undefined`. Real, previously-unknown, worth a small dedicated fix.
