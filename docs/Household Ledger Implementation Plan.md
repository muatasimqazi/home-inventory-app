...
**Bugs found during Wave 2 review — pre-existing Wave 1 code, not Wave 2's scope, not fixed.** Wave 2's `/code-review` pass on Workstream 6 read `store.ts`, `ask/tools.ts`, and `household-setup/page.tsx` as context (Workstream 6's new route touches `item_purchases`/`people`/notification preferences) and surfaced real bugs in that already-merged code, outside the diff actually being reviewed. Fixing them would have meant expanding Wave 2's scope into Wave 1 workstreams' territory well beyond the brief — flagged here instead, for a dedicated pass:
  ...
  - **`removeMember()` doesn't clear the removed member's `people.linked_user_id`** (`store.ts`) — no DB trigger does either, since `linked_user_id` only clears on an `auth.users` row deletion, not household-membership removal. A removed member's Person row keeps showing as a "Managed" profile indefinitely, and the People settings page's delete button for that row now points at the wrong (`deletePerson`) path with materially wrong confirmation copy.
  - **`updatePinnedLocation()` has no rollback if the row update fails after the old photo was already deleted from Storage** (`store.ts`) — `createPinnedLocation` rolls back its own upload on insert failure; this path doesn't, so a network blip on replace/remove can leave a pin's `photo_path` pointing at a deleted object, 404ing permanently until manually re-uploaded.
  - **`addPerson()` is optimistic but its revert-on-failure can desync from a caller that already captured the new id** (`store.ts`) — `AddPersonSheet`'s `onCreated` immediately selects the new person in an ownership picker; if the underlying insert then fails and reverts, the picker's selected id silently points at a person that no longer exists in `people`.
  - **Ask-tool search inputs don't escape `_` for ILIKE queries** (`ask/tools.ts`, at least 5 call sites) — `%`, `,`, `(`, `)` are stripped, but `_` is also a live single-character SQL wildcard, so a query containing one can silently match extra rows it shouldn't (e.g. searching "water_shutoff" would also match "waterXshutoff").
  - **Duplication debt, not a correctness bug (both copies are individually correct after Wave 1's Workstream 3 fix):** the warranty-active NaN-guard logic lives as two separate copies (`item-purchase-section.tsx` and `ask/tools.ts`) instead of one shared helper — a future change to the rule (e.g. a grace period) applied to only one copy would make the UI and the Ask assistant disagree.

  These were surfaced, not independently re-verified line-by-line the way the onboarding join-flow bug was (that one was checked directly against the current code) — treat the rest as strong leads for a focused bug-fix pass, not confirmed-and-triaged the way Wave 1/2's own workstream findings were.
