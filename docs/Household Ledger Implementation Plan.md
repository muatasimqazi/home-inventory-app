...
**Bugs found during Wave 2 review — pre-existing Wave 1 code, not Wave 2's scope, not fixed.** Wave 2's `/code-review` pass on Workstream 6 read `store.ts`, `ask/tools.ts`, and `household-setup/page.tsx` as context (Workstream 6's new route touches `item_purchases`/`people`/notification preferences) and surfaced real bugs in that already-merged code, outside the diff actually being reviewed. Fixing them would have meant expanding Wave 2's scope into Wave 1 workstreams' territory well beyond the brief — flagged here instead, for a dedicated pass:
  ...
  - **Onboarding join flow is likely broken — verified directly, highest priority of this list.** `household-setup/page.tsx`'s zero-household auto-create effect fires unconditionally whenever `mode === "create"` (the default) and the user has no household yet — it only skips for `mode === "join"`, which nothing in the app ever sets automatically. `hydration-gate.tsx` and `sign-in/page.tsx` both route a zero-household user to `/household-setup` with no `?mode=join`, and the "I have an invite code" button only ever renders in the `naming` phase, which is unreachable before a household already exists. Net effect: a user who signed up specifically to redeem a household invite gets a brand-new, unwanted household auto-created and owned by them before they ever see a join option. Worth fixing soon — this is a real, currently-shipped gap in the invite flow, not a hypothetical.
