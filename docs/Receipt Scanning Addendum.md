...
**Statement format support, resolved 2026-08-19** (was listed here as still-open): PDF statement parsing shipped, but scoped narrower than "general PDF statement import" — `lib/vision/extract-statement.ts` + `finance/recurring/import` extracts a statement's transactions specifically to detect *recurring* charges for `RecurringBill`, not to bulk-import every transaction on the statement into the ledger (that remains CSV import's job, Finance PRD §20). A true "photograph or upload any statement and get every transaction extracted as CSV-import-style candidates" feature is still a distinct, unscoped one if wanted later. See `docs/v2-checklist.md` §29 / `docs/bugs.md` #23.

**Email receipts, added 2026-08-19 (not in this document's original scope, added by direct request):** a third capture mode alongside the two below — no photo/PDF at all, a purchase confirmation forwarded to a per-household inbox address instead (`token@receipts.<domain>`, Resend inbound webhook, `lib/vision/extract-email-receipt.ts` extracting from the email's own text). Lands in the identical `receipt_scan_batches`/`scanned_transaction_drafts`/`scanned_receipt_line_items` pipeline this whole document specs — same schema (`receiptSchema`, shared with photo extraction, not a parallel one), same mandatory-review gate, same confirm path (`confirm_scanned_transaction_draft()`). Only real addition: `receipt_scan_batches.source` (`'scan' | 'email'`) so a batch with no photo never reads as a broken scan, and a persistent review surface (`finance/pending-receipts`) for drafts created outside a live scan session, which didn't previously need to exist since every scan-sourced draft was always reviewed synchronously in the same session that created it. Full detail: `docs/v2-checklist.md` §29 / `docs/bugs.md` #22.

## 7. Explicit non-goals, still held

**Line-item *capture* and receipt *retention* are now both fully in scope (§3/§4/§6) — what's still explicitly out of scope is line-item *ledger splitting* and downstream *reporting built on top of the captured detail*:** no per-category budgeting or itemized tax/expense reports driven by `scanned_receipt_line_items` (the data exists and is queryable; building analytics on it is separate, future work, not implied by capturing it now). ~~No PDF statement parsing.~~ **Narrowed, not fully reopened, 2026-08-19** — see "Statement format support, resolved" above: PDF parsing shipped scoped to recurring-bill detection specifically, not general statement-to-ledger import. ~~No bank-sync-adjacent auto-import (Plaid remains explicitly out of scope, Finance PRD §5/§28)~~ **Plaid built 2026-08-19, see `docs/Bank Sync Addendum.md`** — a separate, direct-to-ledger import path (bank-confirmed transactions post without human review, unlike this scanning pipeline's mandatory review step) that coexists with, rather than replaces, this photo/PDF/email-in flow.
