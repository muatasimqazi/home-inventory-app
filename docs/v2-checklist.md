...
**Still open** (not addressed this pass): a PRD-mandated "Leave Household" action (§13) — join/switch/create all exist, leaving one doesn't; and the `label_batches`/`label_batch_entries` `status` lifecycle (`draft`/`generated`/`printed`) from PRD §22, never implemented in either the migration or the mock.
