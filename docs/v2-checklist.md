...
**Still open**: the `label_batches`/`label_batch_entries` `status` lifecycle (`draft`/`generated`/`printed`) from PRD §22, never implemented in either the migration or the mock.
