...
1. ✅ **Feasibility spike (Android)** — done. Real-device testing (not just the emulator) surfaced and fixed three genuine bugs the emulator masked entirely: Google sign-in getting stranded in Chrome (Google refuses to render OAuth inside an embedded WebView — fixed via a Custom Tab + custom-scheme deep link back into the app), the session not surviving a reopen (the real fix turned out to be a missing Supabase Redirect URL wildcard entry, not the WebView cookie-flush timing issue it first looked like), and the app still shipping Capacitor's generic default icon/splash instead of Schuaz's real branding.
2. ✅ **Native push, Android leg** (§2.1) — done, verified end-to-end on a real device: a real FCM token registers on Enable, a real send from the server arrives as an in-app toast when the app is foregrounded and as a real system-tray notification when it isn't, and tapping either correctly opens the app. Along the way, fixed a real gap a live test send surfaced: the passive listeners (registration, tap-handling) only worked while the Settings page happened to be mounted — moved to a globally-mounted listener instead.
3. **Android store readiness** — icons/splash generated (folded into phase 1's cleanup); still needed: permission-string copy review, Play internal testing track, screenshots, store listing, submit.
