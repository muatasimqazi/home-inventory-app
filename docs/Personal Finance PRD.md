...
| **RecurringBill** | id, household_id, name, expected_amount, frequency, next_due_date, category_id, account_id, **owner_user_id**, is_active, trashed_at, permanently_delete_after | Manual only, no detection; `owner_user_id` added 2026-08-18, same nullable joint-vs-personal shape as `Account` |
