...
- **Who can assign a task to whom** — is task assignment open to any member (matching finance's "everyone manages together" model) or does it need any gating at all? Leaning fully open, matching finance's precedent, but stating it explicitly rather than assuming.
