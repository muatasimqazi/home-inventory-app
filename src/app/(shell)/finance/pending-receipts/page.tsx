          ...
          {groups.map((group) => (
            <div key={group.draft.id} className="flex flex-col gap-3 rounded-2xl border border-border bg-white p-4 shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto_auto]">
                <Select value={selectedAccountId[group.draft.id] ?? group.draft.accountId ?? ""} onValueChange={(v) => setSelectedAccountId((s) => ({ ...s, [group.draft.id]: v }))}>
                ...
                <Button variant="outline" onClick={() => handleDismiss(group)} disabled={dismissingId === group.draft.id || confirmingId === group.draft.id}>
                  Dismiss
                </Button>
                <Button onClick={() => handleConfirm(group)} disabled={confirmingId === group.draft.id || dismissingId === group.draft.id}>
