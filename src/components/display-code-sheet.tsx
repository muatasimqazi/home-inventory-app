...
/** View / copy / edit / generate the container's Container ID (displayCode), or claim a preprinted label. */
export function DisplayCodeSheet({ open, onOpenChange, container }: DisplayCodeSheetProps) {
  ...
  async function handleSave() {
    ...
    if (!result.ok) {
      setError(result.error ?? "Couldn't save that Container ID.");
      return;
    }
    toast.success("Container ID saved");
    onOpenChange(false);
  }
  ...
  async function handleGenerateNext() {
    ...
    if (!result.ok) {
      setError(result.error ?? "Couldn't generate a Container ID.");
      return;
    }
    toast.success("Container ID assigned");
    onOpenChange(false);
  }
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="rounded-t-3xl">
        <SheetHeader>
          <SheetTitle className="text-section-title font-medium text-ink">Container ID</SheetTitle>
        </SheetHeader>
