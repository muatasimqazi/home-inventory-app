...
const STYLE_PROMPTS: Record<ItemStudioPhotoStyle, string> = {
  ...
  ghost_mannequin: `Render the item as a realistic "ghost mannequin" (invisible mannequin) ecommerce product photo, shot straight-on from the front — the garment appears filled out with natural human-body volume, shape, and drape (shoulders, sleeves, collar, torso) exactly as it would look worn, but with absolutely no mannequin, model, body, or neck-hole void visible anywhere — a standard apparel-photography technique. Pure white background, even studio lighting. Only apply this treatment when the item is genuinely a body-worn garment (a top, jacket, dress, or similar) where it makes physical sense — if the item is a flat accessory, a shoe, or something that isn't worn draped over a body (a belt, a bag, jewelry), instead render it as a clean, well-lit product shot on a pure white background, exactly like the white_background style, rather than forcing an invisible-mannequin effect that wouldn't make sense for it. ${TRANSFORM_REINFORCEMENT} ${PRESERVE_INSTRUCTION}`,
  // A second angle of the same ghost-mannequin treatment, not another
  // background/lighting variation — meant to pair with ghost_mannequin
  // (front) as its natural companion shot. Explicitly calling for a
  // genuine rotation is the point: every *other* style only changes the
  // background/lighting around whatever angle the source photo already
  // implies, so two of them side by side tended to read as near-duplicate
  // front views rather than two actually different angles of the item.
  ghost_mannequin_profile: `Render the item as a realistic "ghost mannequin" (invisible mannequin) ecommerce product photo, seen from a rotated three-quarter or side profile angle — deliberately NOT a straight-on front view. The garment appears filled out with natural human-body volume, shape, and drape (shoulders, sleeves, collar, torso) exactly as it would look worn from that rotated angle, but with absolutely no mannequin, model, body, or neck-hole void visible anywhere — a standard apparel-photography technique. Pure white background, even studio lighting. This shot is meant to sit alongside a separate front-facing ghost-mannequin photo of the same item as its complementary second angle, so the rotation needs to be obvious and real, not a near-identical front view with a marginal tilt. Only apply this treatment when the item is genuinely a body-worn garment (a top, jacket, dress, or similar) where it makes physical sense — if the item is a flat accessory, a shoe, or something that isn't worn draped over a body (a belt, a bag, jewelry), instead render it as a clean, well-lit product shot on a pure white background from a rotated three-quarter angle, rather than forcing an invisible-mannequin effect that wouldn't make sense for it. ${TRANSFORM_REINFORCEMENT} ${PRESERVE_INSTRUCTION}`,
